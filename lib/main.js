...
class Theme {
  ...
  activate (state) {
    ...
    const reconfigurate = ({newValue, oldValue}) => {
    }
    atom.config.onDidChange('reduced-dark-ui.palette', reconfigurate)
    ...
    for (let task in deferredTasks) {
      setImmediate(() => {
        ...
        try {
          ...
          dev.message('Task failed to run' + JSON.stringify(e)) }
      })
    }
  }
  ...
  appendDependencyToVariablesStyle (n) {
    ...
    let store = atom.getStorageFolder(pkg.name)
    ...
    console.warn("Patching config file in", variablesFilePath);

    try {
      // let start = contents.utf8Slice(0, 100)
      let importCount = 0
      console.warn("trying to find @import strings:", importCount)
      contents.replace(/\@import/g,
        () => {
          console.log(contents, importCount)
          importCount++ })
      console.log(importCount, "inport statements found in the index.less")
      // if (start.search('import') === -1)
      if (importCount < 3)
        fs.writeFileSync(variablesFilePath, `
          @import 'definitions/config-fallback';
