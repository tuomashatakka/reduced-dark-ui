...
import applyColors, { remove as removeStyles } from './core/colors'
...
class Theme {
  ...
  onConfigUpdate (name, value=null, type=null, callback=null) {
    ...
    switch (type) {
      case 'color':
        doc.setAttribute(ns, value.toRGBAString())
        break
...
const activate = state => {
  ...
  subscriptionManager.subscribe('decorate', decorate, { initial: true })

  if (atom.devMode)
    console.log("subs", subs, subscriptionManager)

  applyColors()
}

const decorate = () => decoratePanels()
