...
export const
  ...
  observeSettingsPanel = () => {
    ...
    atom.packages.onDidDeactivatePackage(
      (pkg) => pkg.name === 'settings-view' && subscriptions ? subscriptions.dispose() : null)
  },
  ...
  extendCallbackFor = ({item, name}) => {
    ...
    if (atom.devMode) console.log("Overriding the settings view theme page creation callback");
    ...
    const getCustomThemeView = () => {
      try {
        ...
        let initial         = { readme: readme.get(0), details }
