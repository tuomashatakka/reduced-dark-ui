...
const
      ...
      updateFontLists = (fp=null) => {
        let pack = atom.packages.getActivePackage('reduced-dark-ui')
        if (!pack)
          return
        ...
        http.get(url, res => {
          ...
          res.on('end', () => {
            ...
            let targetPath = path.join(pack.path, 'assets', 'fonts', 'google.fonts.json')
            fs.writeFileSync(targetPath, JSON.stringify(received, null, 2))
          })
        })
