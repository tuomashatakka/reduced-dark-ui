...
const applyConfig = (...args) => applyStylesheet(...args)
const applyFonts = (...args) => applyStylesheet(...args)
...
const provideConfigToLess = (rules={}) => {
  ...
  const applicate = () => {
    ...
    let fontRequest =
      getFontFaceDefinition(values.uiFont)

    setImmediate(() =>
      applyConfig(pack, output, 'config'))

    console.log(fontRequest);
    if (fontRequest)
      fontRequest
        .then(response => response.raw && applyFonts(pack, response.raw, 'font'))
        .catch(e => dev.message(e))
  }

  let timestamp = new Date().getTime()
  let diff = timestamp - lastFetchedTimestamp

  clearTimeout(applicate)
  if (diff < 3000)
    setTimeout(applicate, diff)
  else
    applicate()
