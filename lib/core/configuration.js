...
import { appendFontDependency } from './fonts'
...
const applyStylesheet = debounce((pack, output, file='config') => {
...
const provideConfigToLess = (rules={}) => {
  ...
  let output =
    generateLessVariableSet([confrules])

  let fontRequest =
    appendFontDependency(confrules.uiFont)
  ...
  fontRequest
    .then(
      ...
      fontFace = fontFace.raw
      applyStylesheet(pack, fontFace, 'font')
    })
    .catch(
      e => dev.message(e))

    setImmediate(
      () => applyStylesheet(pack, output, 'config'))
}
