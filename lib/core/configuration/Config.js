...
export default class ConfigManager extends Emitter {
  ...
  didFinishTransaction (name) {
    ...
    if (next && next.content) {
      ...
      let source
      try {
        source = atom.themes.loadStylesheet(path) }
      catch (e) {
        this.initStorage() }
  ...
  writeConfigurationToFile ({ name, content, path }) {
    setImmediate(() => this.storageInitialized ?
      writeFile(path, content, 'utf8',
      () => this.didFinishTransaction(name, content))
      : setTimeout(() => this.writeConfigurationToFile({name, content, path}), 500)
    )
  }
  ...
  initStorage () {
    ...
    this.init.then(() => {
      ...
      this.requestConfigurationRewrite({name:'config'})
    })
  }
