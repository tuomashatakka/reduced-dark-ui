...
const COLOR_PROCESSORS = {
  ...
  backgroundBase: function(color) {
    color = CF.multiply(this.get('tint'), color, 0.95)
    color = CF.desaturate(color, 0.5)
    return CF.dim(color, 0.1) },

  backgroundSub: function(color) {
    color = CF.multiply(color, this.get('tint'), 0.94)
    color = CF.desaturate(color, 0.4)
    return CF.dim(color, 0.6) },

  backgroundDark: function(color) {
    color = CF.multiply(color, this.get('tint'), 0.94)
    color = CF.desaturate(color, 0.3)
    return CF.dim(color, 0.4) },

  backgroundLight: function(color) {
    color = CF.multiply(color, this.get('tint'), 0.8)
    color = CF.dim(color, 0.2)
    return CF.saturate(color, 0.5) },

  backgroundNeutral: function(color) {
    color = CF.multiply(color, this.get('tint'), 0.15)
    color = CF.desaturate(color, 0.5)
    return CF.lighten(color, 0.05) },

  backgroundMedium: function(color) {
    color = CF.multiply(color, this.get('tint'), 0.85)
    color = CF.dim(color, 0.2)
    return CF.saturate(color, 0.15) },
