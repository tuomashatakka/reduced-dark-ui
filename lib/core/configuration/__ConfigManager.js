...
export default class ConfigManager extends Emitter {
  ...
  constructor (args) {
    ...
    let storagePath       = !atom.inSpecMode() ?
                            store.pathForKey(dirName) :
                            baseDirForSpecs
    this.queue            = []
    ...
    this.configured       = !atom.inSpecMode() ?
                            localStorage.getItem('reduced-dark-activated') === "1" :
                            false
    // Shorthands
    this.updateFont = (content) =>
      this.requestConfigurationRewrite({ name: 'font',   content })
    this.updateConf = (content) =>
      this.requestConfigurationRewrite({ name: 'config', content })

    // Event listeners
    let mutationEvent = this.on(OPERATION.ON_CHANGE, payload => {
      this.transaction = payload ? true : false

      console.warn("OPERATION.ON_CHANGE", this.transaction, payload)
      console.warn(this.transaction ? "OPERATION.BEGIN" : "OPERATION.END")

      if(this.transaction)
        this.emit(OPERATION.ON_DID_BEGIN, payload)
      else
        this.emit(OPERATION.ON_DID_END, payload)
    })

    let startEvent  = this.on(OPERATION.ON_DID_BEGIN,
        data => this.writeConfigurationToFile(data))

    let endEvent    = this.on(OPERATION.ON_DID_END,
        data => this.processQueue(data))

    let finishEvent = this.on(OPERATION.ON_STACK_EMPTIED, () => {
      console.warn("OPERATION.ON_STACK_EMPTIED")
      // this.compiler.compileDeferred()
      // if (atom.inSpecMode()) {
      //   this.compiler.apply(new File(baseDirForSpecs + '/__index.less'))
      // } else {
      // let entries   = this.storage.getEntriesSync()
      // console.log("entries in da storidz",entries)
  ...
  requestConfigurationRewrite ({ content, name }) {
    ...
    this.emit(OPERATION.REQUEST_REWRITE, payload)
  ...
  processQueue (payload) {
    ...
    let next = null
    console.warn("processQueue with payload\n", payload)
    if (payload) {
    ...
    console.warn("processQueue.queue -\n", this.transaction, [...this.queue])
    if (this.transaction)
      return
