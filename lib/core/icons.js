...
import { applyIconfont } from './configuration/index'
...
export const findElementIcon = (el) => {
    ...
    while (!match && haystack.length) {
      ...
      cls.replace(cond, (_, name) => icon = name)
    }
    return { iconset, icon }
  },


  makeIcon = ({ node, iconset, icon, className }) => {
    let el    = node || document.createElement('span')
    el.setAttribute('class', `${iconset} ${iconset}-${icon} ${className}`)
    return el
  },
  ...
  provideIconfonts = () => {
    let basePath = atom.packages.getLoadedPackage(PACKAGE_NAME).path
    Task.once(resolve(`${basePath}/lib/proc/provide-icon-fonts`), `${basePath}`, data => {
      applyIconfont('ionicons', data.ionicons)
    })
  },
